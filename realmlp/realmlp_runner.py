...
from realmlp.realmlp_model import RealMLPStyleModel


DEFAULT_REALMLP_FEATURES = [
    "TransactionAmt",
    "LogAmt",
    "Hour",
    "DayOfWeek",
...
def base_preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    num_cols = [c for c in DEFAULT_REALMLP_FEATURES if c in df.columns]
    for c in num_cols:
...
def build_fold_tabular(df_tr: pd.DataFrame, df_vl: pd.DataFrame) -> FoldTabularBundle:
    feature_cols = [c for c in DEFAULT_REALMLP_FEATURES if c in df_tr.columns and c in df_vl.columns]
    if not feature_cols:
        raise ValueError("No RealMLP feature columns available")
...
def train_fold_realmlp(
    ...
    *,
    ...
    print(f"[REALMLP] Fold {fold_index + 1}/{n_splits}")
    print(f"[REALMLP] Train: {len(df_tr)}, Val: {len(df_vl)}")

    bundle = build_fold_tabular(df_tr, df_vl)
    print(f"[REALMLP] feature_cols ({len(bundle.feature_cols)}): {bundle.feature_cols}")

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    if device.type == "cuda":
        print(f"[REALMLP] Device: cuda ({torch.cuda.get_device_name(0)})")
    else:
        print("[REALMLP] Device: cpu")
    ...
    for epoch in range(epochs):
        ...
        print(f"[REALMLP] Epoch {epoch + 1}/{epochs} loss={np.mean(losses):.4f}")
